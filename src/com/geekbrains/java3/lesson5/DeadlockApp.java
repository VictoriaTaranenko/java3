package com.geekbrains.java3.lesson5;
...
public class DeadlockApp {
    ...
    private static class DeadThreadOne extends Thread {
        public void run() {
            synchronized (lock1) { // 1. захватываем монитор
                System.out.println("DeadThreadOne is holding LOCK 1...");
                try {
                    Thread.sleep(1000); // 2. спим
    ...
    private static class DeadThreadTwo extends Thread {
        public void run() {
            synchronized (lock2) { // 3. пока спит тот поток захватываем монитор
